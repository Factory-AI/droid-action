...
- When in doubt about whether something is a bug, report it. Missing a real bug is far worse than a false positive. A downstream validator will filter out false positives, so err on the side of reporting. Avoid purely stylistic nitpicks, but do report any issue where the code could behave incorrectly.
  </review_guidelines>
