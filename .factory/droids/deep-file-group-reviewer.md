...
### Also flag (these are valid findings):
- Method name typos, especially in test methods where a typo means the test never runs
- Docstring/comment mismatches that indicate the code was changed but docs weren't updated
- i18n/translation errors (wrong language in locale file)
- Dead code that indicates an incomplete refactoring
- Log level misuse (Error for debug info, Debug for errors)
...
- `body`: Comment text starting with priority tag [P0|P1|P2], then title, then 1 paragraph explanation
  ...
  - P2: Minor bugs (edge cases, naming errors, documentation mismatches)
...
- `confidence`: "high" if you verified the issue against cross-file context, "medium" if based on diff alone
</output_format>
