...
} from "../src/github/operations/comments/update-droid-comment";

describe("updateDroidComment", () => {
  ...
  test("should update issue comment successfully", async () => {
    ...
    const params: UpdateDroidCommentParams = {
      owner: "testowner",
      repo: "testrepo",
      commentId: 123456,
      body: "Updated comment",
    ...
    const result = await updateDroidComment(mockOctokit, params);
  ...
  test("should propagate error when PR review comment update fails with non-404 error", async () => {
    ...
    await expect(updateDroidComment(mockOctokit, params)).rejects.toEqual(
      mockError,
    );
