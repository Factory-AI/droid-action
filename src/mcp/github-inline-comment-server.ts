...
// Droid can't accidentally approve a PR
...
server.tool(
  ...
  "Create an inline comment on a specific line or lines in a PR file. Always set the 'side' parameter (RIGHT for added/modified code, LEFT only for removed code)",
  {
    path: z
      .string()
    ...
    line: z
      ...
      .optional()
      .describe(
        "Line number for single-line comments (required if startLine is not provided). Line numbers must correspond to the specified diff side (RIGHT side targets new code, LEFT side targets removed code).",
      ),
    startLine: z
      .number()
      .nonnegative()
      .optional()
      .describe(
        "Start line for multi-line comments (use with line parameter for the end line). Line numbers must correspond to the specified diff side (RIGHT side targets new code, LEFT side targets removed code).",
      ),
    side: z
      .enum(["LEFT", "RIGHT"])
      .optional()
